...
def main(args):
    ...
    print("model_save_path:", model_save_path, os.path.exists(model_save_path))
    print("pretrained_model:", pretrained_model, os.path.exists(pretrained_model))
    #prepare miou fun
    ...
    if args['model']['polar']:
        train_dataset=spherical_dataset(train_pt_dataset, args['dataset'], grid_size = grid_size, ignore_label = 0, use_aug = False)
    ...
    while epoch < args['model']['max_epoch']:
        ...
        for i_iter,(train_vox_fea,train_label_tensor,train_gt_center,train_gt_offset,train_grid,_,_,train_pt_fea) in enumerate(train_dataset_loader):
            ...
            try:
                ...
                # forward
                if visibility:
                    sem_prediction,center,offset,cluster_fea = my_model(train_pt_fea_ten,train_grid_ten,train_vox_fea_ten)
                else:
                    sem_prediction,center,offset,cluster_fea = my_model(train_pt_fea_ten,train_grid_ten)
                # loss

                B, C, H, W = cluster_fea.shape
                cluster_fea = cluster_fea.permute(0, 2, 3, 1).contiguous()
                cluster_fea = cluster_fea.view(B, H * W, C)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained_model', default='pretrained_weight/Panoptic_SemKITTI_PolarNet.pt')
