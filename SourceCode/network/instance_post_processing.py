...
def get_panoptic_segmentation(sem, ctr_hmp, offsets, thing_list, label_divisor=2**16, void_label=0,
    ...
    if sem.dim() == 5:
        ...
        semantic = sem.type(torch.ByteTensor)
        # shift back to original label idx 
        semantic = torch.add(semantic, 1).type(torch.LongTensor)
        one_hot = torch.zeros((sem.size(0),torch.max(semantic).item()+1,sem.size(1),sem.size(2),sem.size(3)))
